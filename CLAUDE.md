...
- **`ShapeSource` re-serialises its whole feature collection on every render, and cannot memoise that away itself** — its children are fresh elements each time, so the `memo` the binding ships never holds. Keep each source in its own memoised component (`StationsSource`, `RoutesSource`) with referentially stable props, or selecting a stop — which changes only the routes — re-stringifies several thousand stations for a string React then diffs away. The web twin hands MapLibre GL JS the object through `setData` and pays nothing, so this only ever shows up on a device.
- **A list built inside the commit that answers a tap must be virtualised.** A well-served stop carries hundreds of departures and each row draws two `react-native-svg` glyphs; mounting them all before anything can paint is a visible beat on iOS, where the same rows are free as DOM nodes on web. `ModalList` is the virtualised sheet body — `ModalScrollView` is only for bodies of a known, small size.
- **Touch targets are 44pt, and body text does not get there on its own.** A row of `text.body` plus `spacing.xSmall` padding is 39pt: fine under a mouse, unreliable under a thumb. Give a tappable row `minHeight: spacing.large`.
