...
<!-- Vendored from https://github.com/danyuchn/asd-ste100-skill (MIT, see LICENSE) -->
...
The `SessionStart` hook in `.claude/settings.json` loads this file into every session, so it is
standing guidance and not only an on-demand rewriter.

- The `Writing` section of `AGENTS.md` wins where the two differ. It is stricter on length and
  structure: one claim per sentence, a table instead of prose for a mapping, no preamble and no
  closing summary that repeats the diff.
- Default to STE-flavored. Docs, TSDoc comments, plans, PR bodies and chat answers are prose,
  and the one-word-one-meaning lockdown reads as a personality transplant on them.
- Use Strict where a wrong reading has a cost and nobody is there to ask: a rule written into an
  `AGENTS.md`, or the scoped job handed to a subagent.
- `Output Format` governs only a text the user hands over to rewrite. A normal answer stays a
  normal answer.
- Commit messages follow the `Git` section, not this skill.
